"use client";

import { useEffect, useRef } from "react";
import type { TradeRecord } from "@/hooks/useSimulation";

interface TradeFeedProps {
  trades:    TradeRecord[];
  isReplay?: boolean;
}

function fmtTime(simTime: number): string {
  if (!simTime || !isFinite(simTime)) return "—";
  if (simTime > 86400) {
    return new Date(simTime * 1000).toISOString().slice(11, 19);
  }
  const m = Math.floor(simTime / 60);
  const s = (simTime % 60).toFixed(1).padStart(4, "0");
  return `${m}:${s}`;
}

function fmtPrice(price: number): string {
  if (!price || !isFinite(price)) return "—";
  return price >= 1000 ? price.toFixed(2) : price.toFixed(4);
}

function fmtQty(qty: number, isReplay: boolean): string {
  if (!qty || !isFinite(qty)) return "—";
  if (isReplay) return qty.toFixed(6).replace(/0+$/, "").replace(/\.$/, "");
  return String(qty);
}

// ── Replay: tape-style market trades ─────────────────────────────────────────

function ReplayTradeFeed({ trades }: { trades: TradeRecord[] }) {
  const mktTrades = trades.filter((t) => !t.is_agent).slice(-50);
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = scrollRef.current;
    if (el) el.scrollTop = el.scrollHeight;
  }, [mktTrades.length]);

  return (
    <div className="flex flex-col h-full overflow-hidden font-mono text-[0.6rem]">
      <div className="grid grid-cols-[4.5rem_3rem_4.5rem_5rem] gap-1 px-2 py-0.5 border-b border-[#111] text-[#333] uppercase shrink-0">
        <span>TIME</span>
        <span>SIDE</span>
        <span className="text-right">QTY BTC</span>
        <span className="text-right">PRICE</span>
      </div>

      {mktTrades.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-[#333] tracking-widest">
          AWAITING TRADES
        </div>
      ) : (
        <div ref={scrollRef} className="flex-1 overflow-y-auto">
          {mktTrades.map((t, i) => {
            const isBuy     = t.side === "buy";
            const color     = isBuy ? "#00d26a" : "#ff3b3b";
            return (
              <div
                key={i}
                className="grid grid-cols-[4.5rem_3rem_4.5rem_5rem] gap-1 px-2 py-[3px] items-center border-b border-[#0d0d0d]"
                style={{ borderLeftColor: color, borderLeftWidth: 2, borderLeftStyle: "solid" }}
              >
                <span className="text-[#444] tabular-nums">{fmtTime(t.sim_time)}</span>
                <span style={{ color }} className="font-bold uppercase">{isBuy ? "BUY" : "SELL"}</span>
                <span style={{ color }} className="text-right tabular-nums">{fmtQty(t.quantity, true)}</span>
                <span style={{ color }} className="text-right tabular-nums">{fmtPrice(t.price)}</span>
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
}

// ── Simulate: agent fills ─────────────────────────────────────────────────────

export default function TradeFeed({ trades, isReplay = false }: TradeFeedProps) {
  if (isReplay) return <ReplayTradeFeed trades={trades} />;

  const agentTrades = trades.filter((t) => t.is_agent);
  const scrollRef   = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = scrollRef.current;
    if (el) el.scrollTop = el.scrollHeight;
  }, [agentTrades.length]);

  if (agentTrades.length === 0) {
    return (
      <div className="flex flex-col h-full overflow-hidden font-mono text-[0.6rem]">
        <div className="grid grid-cols-[2.5rem_3.5rem_5rem_2.5rem] gap-1 px-2 py-0.5 border-b border-[#111] text-[#333] uppercase shrink-0">
          <span>TIME</span>
          <span>SIDE</span>
          <span className="text-right">PRICE</span>
          <span className="text-right">QTY</span>
        </div>
        <div className="flex-1 flex items-center justify-center text-[#333] tracking-widest">
          NO FILLS YET
        </div>
      </div>
    );
  }

  return (
    <div className="flex flex-col h-full overflow-hidden font-mono text-[0.6rem]">
      <div className="grid grid-cols-[2.5rem_3.5rem_5rem_2.5rem] gap-1 px-2 py-0.5 border-b border-[#111] text-[#333] uppercase shrink-0">
        <span>TIME</span>
        <span>SIDE</span>
        <span className="text-right">PRICE</span>
        <span className="text-right">QTY</span>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        {agentTrades.map((t, i) => {
          const sideColor = t.side === "buy" ? "#00d26a" : "#ff3b3b";
          return (
            <div
              key={i}
              className="grid grid-cols-[2.5rem_3.5rem_5rem_2.5rem] gap-1 px-2 py-0.5 items-center border-b border-[#0f0f0f] border-l-2 border-l-[#3b82f6]"
            >
              <span className="text-[#444] tabular-nums">{fmtTime(t.sim_time)}</span>
              <span style={{ color: sideColor }} className="font-bold uppercase">{t.side}</span>
              <span style={{ color: sideColor }} className="text-right tabular-nums">{fmtPrice(t.price)}</span>
              <span className="text-right text-[#888] tabular-nums">{t.quantity}</span>
            </div>
          );
        })}
      </div>
    </div>
  );
}
